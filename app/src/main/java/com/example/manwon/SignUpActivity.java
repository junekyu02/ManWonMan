            ...
            if (!nickname.isEmpty()) {
                ...
                usersRef.orderByChild("nickname").equalTo(nickname).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        // Firebase 에러 처리
                        Toast.makeText(SignUpActivity.this, "닉네임 확인 중 오류 발생: " + error.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            } else {
                binding.nicknameMessage.setText("닉네임을 입력해 주세요.");
                ...
                mAuth.createUserWithEmailAndPassword(email, password)
                        .addOnCompleteListener(this, task -> {
                            if (task.isSuccessful()) {
                                ...
                                // Firebase Realtime Database에 닉네임 저장
                                String uid = FirebaseAuth.getInstance().getCurrentUser().getUid();
                                DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("users").child(uid);
                                userRef.child("nickname").setValue(nickname).addOnCompleteListener(nicknameTask -> {
                                    if (nicknameTask.isSuccessful()) {
                                        ...
                                        // 닉네임 저장 실패
                                        Toast.makeText(SignUpActivity.this, "회원가입은 성공했지만 닉네임 저장에 실패했습니다.", Toast.LENGTH_SHORT).show();
                                    }
                                });
                            } else {
                                // 회원가입 실패
